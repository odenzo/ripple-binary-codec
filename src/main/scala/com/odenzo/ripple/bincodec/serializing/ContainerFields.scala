...
import com.odenzo.ripple.bincodec.serializing.TypeSerializers.{
  encodeFieldAndValue,
  encodeHash256,
  json2array,
  json2object
...
trait ContainerFields extends StrictLogging with SerializerUtils {
  ...
  def encodeSTArray(data: FieldData, isSigning: Boolean): Either[RippleCodecError, NestedEncodedValues] = {
    ...
    val allAnwers: Either[RippleCodecError, List[FieldEncoded]] = nested.flatMap(_.traverse(handleOneVar(_, isSigning)))

    val noMetaData: Either[RippleCodecError, List[List[BinarySerializer.Encoded]]] =
