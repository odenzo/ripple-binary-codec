...
trait ContainerFields extends StrictLogging with SerializerUtils {
  ...
  /**
    ...
    */
  def encodeSTObject(o: Json, isNested: Boolean, isSigning: Boolean): Either[CodecError, NestedEncodedValues] = {
    ...
    val jobj: Either[CodecError, JsonObject] = json2object(o)

    // This will filter and order the fields
    val fields: Either[CodecError, List[FieldData]] = jobj.flatMap(prepareJsonObject(_, isSigning))

    val ans: Either[CodecError, List[FieldEncoded]] =
  ...
  /**
    ...
    * @return
    */
  def prepareJsonObject(o: JsonObject, isSigning: Boolean): Either[CodecError, List[FieldData]] = {
    logger.trace(s"prepareJsonObect ")
    val bound: Either[CodecError, List[FieldData]] = o.toList.traverse {
    ...
    val sorted: Either[CodecError, List[FieldData]] = filtered.map(_.sortBy(_.fi.sortKey))

    sorted
  }

  def encodeSTArray(data: FieldData, isSigning: Boolean): Either[CodecError, NestedEncodedValues] = {
    ...
    /**
      ...
      * @return
      */
    def handleOneVar(v: JsonObject, isSigning: Boolean): Either[CodecError, FieldEncoded] = {
      val asList = v.toList
      if (asList.length != 1) CodecError("Expected Exaclty One Field", v.asJson).asLeft
      else {
    ...
    val arr: Either[CodecError, List[Json]]                            = json2array(data.v)
    val nested: Either[CodecError, List[Either[CodecError, JsonObject]]] = Nested(arr).map(json2object).value
    val objArray: Either[CodecError, List[JsonObject]]                 = nested.flatMap(_.sequence)

    val allAnwers: Either[CodecError, List[FieldEncoded]] = objArray.flatMap(_.traverse(handleOneVar(_, isSigning)))

    val noMetaData: Either[CodecError, List[List[BinarySerializer.Encoded]]] = Nested(allAnwers).map(_.encoded).value




    val foo: Either[CodecError, List[List[Encoded]]]       = noMetaData.map((v: List[List[Encoded]]) => v :+ List(DefinitionData.arrayEndMarker))
    val bar: Either[CodecError, List[NestedEncodedValues]] = Nested(foo).map(NestedEncodedValues).value
  ...
  *   This is usually a field, maybe always not sure.
    ...
    * @return
    */
  def encodeVector256(data: FieldData): Either[CodecError, NestedEncodedValues] = {
    // Like Indexes, these all seems to be 64 hex bytes in quotes 256 bits

    def encodeListOfHash(jsonList: List[Json]): Either[CodecError, List[BinarySerializer.RippleTypeEncoded]] =
