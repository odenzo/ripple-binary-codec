...
/**
  ...
  *       jmh:run  -i 10 -wi 10 -f1 -t1  BenchmarkCodecSignRq
  *
  */
...
//    "-XX:FlightRecorderOptions=defaultrecording=true,dumponexit=true,dumponexitpath=/tmp/mapOnFunctionsAndList.jfr"))
  ...
  def singers(bh: Blackhole): Unit = {
    val rs = sign(randomSign())
    bh.consume(rs)
  }

  def sign(tx_json: Json): Either[BinCodecLibError, Array[Byte]] = RippleCodecAPI.signingTxBlob(tx_json)
}
