...
class SignFor$Test extends OTestSpec {
  ...
  test("Correct Serialization and Hash") {
    ...
    val signingEncoded = getOrLog(RippleCodecAPI.signingTxBlob(tx_json).map(ByteVector(_)))
    logger.debug(s"Signing ${signingEncoded.toHex}")

    val binaryEncoded = getOrLog(RippleCodecAPI.serializedTxBlob(tx_json).map(ByteVector(_)))
    logger.debug(s"Binary ${binaryEncoded.toHex}")
