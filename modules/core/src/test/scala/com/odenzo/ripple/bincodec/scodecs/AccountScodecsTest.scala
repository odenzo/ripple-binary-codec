...
class AccountScodecsTest extends OTestSpec with AccountScodecs {
  ...
  test("Address Encode") {
    encodecChecksummedAccount(x).map { r =>
  ...
  test("Address Decoder") {
    ...
    val res    = xrpAddressDecoder.decode(binary)
    scribe.info(s"Res $res")
  }

  test("RoundTrip") {
    val x                         = "r9cZA1mLK5R5Am25ArfXFmqgNwjZgnfk59"
    val enc: BitVector            = encodecChecksummedAccount(x).require
    val dec: DecodeResult[String] = xrpAddressDecoder.decode(enc).require
