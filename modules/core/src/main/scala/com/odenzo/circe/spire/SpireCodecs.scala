package com.odenzo.circe.spire
...
//  // Meh, this is recursive and not happy.
...
//
//    Decoder
//      .instance(_.downField("Long").as[Long].map(Algebraic.apply))
//      .or(Decoder.instance(_.downField("Double").as[Double].map(Algebraic.apply)))
//      .or(Decoder.instance(_.downField("BigDecimal").as[BigDecimal].map(Algebraic.apply)))
//      .or(Decoder.instance(_.downField("Rational").as[Rational].map(Algebraic.apply)))
//      .or(Decoder.instance(_.downField("Root").as(constantRootDecoder).map((Algebraic.unsafeRoot _).tupled)))
//      .or(Decoder.instance(_.downField("Neg").as[Algebraic].map(_.unary_-)))
...
object SpireCodecs extends SpireCodecs
