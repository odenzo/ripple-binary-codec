...
/**
  ...
  * From: https://github.com/scodec/scodec-spire/blob/master/src/main/scala/scodec/interop/spire/package.scala#L15
  */
package object spire {
  ...
  implicit class SpireCodecEnrichment[A](val codec: Codec[A]) extends AnyVal {
    ...
    def toUByte(implicit ev: A =:= Byte): Codec[UByte] =
      codec
        .xmap[UByte](b => UByte(b), _.toByte.asInstanceOf[A])
        .withToString(codec.toString)

    def toUShort(implicit ev: A =:= Short): Codec[UShort] =
      codec
        .xmap[UShort](s => UShort(s), _.toShort.asInstanceOf[A])
        .withToString(codec.toString)

    def toUInt(implicit ev: A =:= Int): Codec[UInt] = codec.xmap[UInt](i => UInt(i), _.toInt.asInstanceOf[A]).withToString(codec.toString)

    def toULong(implicit ev: A =:= Long): Codec[ULong] =
      codec
        .xmap[ULong](l => ULong(l), _.toLong.asInstanceOf[A])
        .withToString(codec.toString)
